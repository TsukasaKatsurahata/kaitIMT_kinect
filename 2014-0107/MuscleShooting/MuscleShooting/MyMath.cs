...
namespace MuscleShooting
{
    class MyMath
    {
        ...
        public static float Collision(float fx, float fy, float tx, float ty, float vx, float vy, float vtx, float vty, float L) {
            double tt1, tt2;
            double xx = (double)(tx - fx), yy = (double)(ty - fy), vxx = (double)(vtx - vx), vyy = (double)(vty - vy);
	        double col = -vxx*vxx*yy*yy -vyy*vyy*xx*xx + 2*vxx*vyy*xx*yy + L*L*vxx*vxx + L*L*vyy*vyy;
            if (col >= 0) {
                tt1 = (-vxx * xx - vyy * yy - Math.Sqrt(col)) / (vxx * vxx + vyy * vyy);
                tt2 = (-vxx * xx - vyy * yy + Math.Sqrt(col)) / (vxx * vxx + vyy * vyy);
        ...
        public static float Collision(Point from, Point Target, Point Vector, Point VectorT, float L) {
            return Collision((float)from.X, (float)from.Y, (float)Target.X, (float)Target.Y, (float)Vector.X, (float)Vector.Y, (float)VectorT.X, (float)VectorT.Y, L);
        }
        public static float Collision(float fx, float fy, float tx, float ty, float vx, float vy, float L, Point ellipse) {
            double ex = ellipse.X, ey = ellipse.Y;
            double tt1, tt2;
        ...
        public static float Collision(Point from, Point Target, Point Vector, float L, Point ellipse) {
            return Collision((float)from.X, (float)from.Y, (float)Target.X, (float)Target.Y, (float)Vector.X, (float)Vector.Y, L, ellipse);
        }
    }
}
